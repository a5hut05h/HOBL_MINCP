...
def parse_ipcm_power(run_dir: Path) -> list[dict]:
    """Parse optional IPCM average rail power readings in milliwatts."""
    filepath = _find_file(run_dir, "IPCM/ipcmview-sum-*.csv")
    if filepath is None:
        return []
    ...
    try:
        with open(filepath, "r", encoding="utf-8-sig", newline="") as f:
            ...
            if reader.fieldnames is None:
                logger.warning("IPCM summary is empty: %s", filepath)
                return []

            headers = {header.strip(): header for header in reader.fieldnames}
            channel_header = headers.get("Channel Name")
            power_header = headers.get("Power Average (mW)")
            if channel_header is None or power_header is None:
                logger.warning("Unexpected IPCM summary headers in %s: %s", filepath, reader.fieldnames)
                return []

            for row in reader:
                name = (row.get(channel_header) or "").strip()
                value_raw = (row.get(power_header) or "").strip()
                if not name:
                    continue
                try:
                    value = float(value_raw)
                except ValueError:
                    logger.warning("Invalid IPCM power value for rail %s: %s", name, value_raw)
                    continue
                metrics.append({"name": name, "value": value, "unit": "mW"})
    except OSError as e:
        logger.error("Failed to read %s: %s", filepath, e)
        return []

    logger.info("Parsed %d IPCM power metrics from %s", len(metrics), filepath.name)
    return metrics
...
def parse_perf_metrics(run_dir: Path) -> list[dict]:
    ...
    Returns list of {"name": str, "value": float, "unit": "ms"} to match the
