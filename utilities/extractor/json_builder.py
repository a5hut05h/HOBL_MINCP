...
def extract_run(run_dir: Path) -> dict | None:
    ...
    ipcm_power = parse_ipcm_power(run_dir)
    ...
    if ipcm_power:
        result["ipcm_power_metrics"] = ipcm_power

    logger.info("Extracted run %s: %d power metrics, %d IPCM power metrics, "
                "%d perf metrics, %d ETL/trace files",
                run_dir.name, len(power), len(ipcm_power), len(perf), len(etl))
    return result
